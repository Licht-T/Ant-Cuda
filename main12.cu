...
int main(int argc, char *argv[]){
    ...
    double foodsvol[MACRO_MAX_TIME][MACRO_NUM_FOODS];
    ...
    for(unsigned long long int dummy=1; dummy<=MACRO_MAX_STEP; dummy++){
        ...
        std::string foodofsname = fool_num_food;
        foodofsname.replace(fool_num_food.find("deg_food"), 8, std::string("deg_food_n"+toString(500)+"_as"+toString(0)+"_ens"+toString(dummy)));

        std::ofstream ofsfood(foodofsname.c_str());
        IOFoodAmountWrite(ofsfood, foodsvol);
    }
    ...
    for (int pw=1; pw<=7; pw++){
        ...
        for (int n=0; n<=450; n+=50){
            ...
            for(unsigned long long int dummy=1; dummy<=MACRO_MAX_STEP; dummy++){
                ...
                std::string foodofsname = fool_num_food;
                foodofsname.replace(fool_num_food.find("deg_food"), 8, std::string("deg_food_n"+toString(n)+"_as"+toString(pw)+"_ens"+toString(dummy)));
